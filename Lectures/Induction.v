...
(** **** Exercise: 1 star, standard, optional (destruct_induction) 
    ...
    - décomposition contre récurrence.
...
(** **** Exercise: 2 stars, advanced, especially useful (plus_comm_informal) 
    ...
    Proof: (* FILL IN HERE *)
    fix HR 1. Raisonnons par récurrence sur le premier argument n.
    intros n m. Soient n et m deux entiers naturels. 
    case n as [ | pn ]. Examinons les deux cas pour n.
    - simpl. apply plus_n_O. Pour n := 0, appliquer la proposition que 0 et m commutent.
    - Considérons le cas n := (S pn).
      simpl. Simplifion.
      case (plus_n_Sm m pn). Récrivons la partie droite pour mettre le S en tête.
      case (HR pn _). Récrivons en utilisant l'hypothèse de récurrence en pn.
      reflexivity. Concluons par réflexivité. 
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal) 
    ...
    Examinons les deux cas pour n.
    - Pour n := 0, c'est bien égal.
    - Pour n := S pn, aussi en utlisant l'hypothèse de récurrence.
...
Proof.
  ...
  intros n m p.
  case (plus_comm p n).
  case (plus_comm (m + p) n).
  case (plus_assoc m p n).
  reflexivity.
Qed.
...
Proof.
  ...
  case m as [ | pm].
  {
    ...
    Check eq_sym.
    Check mult_0_r.
    case (eq_sym (mult_0_r n)).
    reflexivity.
  }
  {
    Search(_ * (S _)).
    simpl.
    assert(forall x y, x + x * y = x * (S y)) as Mult_1_S2.
    {
      fix HRL 1.
      intros x y.
      case x as [ | px ].
...
Theorem andb_false_r : forall b : bool,
    andb b false = false. (* analyse de cas *)
Proof.
  (* FILL IN HERE *)
  intro b.
  case b; reflexivity.
...
Proof.
  (* FILL IN HERE *) (* induction sur p (à cause de l'addition) *)
  fix HR 3.
  intros n m p NM.
  case p as [ | pp ].
  - exact NM.
...
Proof.
  (* FILL IN HERE *) (* calcul + récriture via lemme utilisant une induction *)
  intro n.
  simpl.
  Search (_ + 0).
  Print plus_n_O.
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
  (* FILL IN HERE *) (* analyse de cas *)
  intros b c; case b; case c; reflexivity.
Qed.
  
Theorem mult_plus_distr_r : forall n m p : nat,
  (n + m) * p = (n * p) + (m * p).
Proof.
  (* FILL IN HERE *) (* induction sur n *)
  fix HR 1.
  intros n m p.
  case n as [ | pn ].
  - reflexivity.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    case (eq_sym (HR pn m p)).
    case (mult_plus_distr_r m (pn * m) p).
    reflexivity.
Qed.
...
Proposition morphismeZero_bin_to_nat :
  bin_to_nat zero_bin = 0.
Proof.
  reflexivity.
Qed.

Theorem morphismeSuccesseur_bin_to_nat :
  forall b, bin_to_nat (S_bin b) = S (bin_to_nat b).
Proof.
  fix HR 1.
  intro b.
  case b as [ | b0 | b1 ].
...
Lemma morphismeZero_nat_to_bin :
  nat_to_bin 0 = zero_bin.
Proof.
  reflexivity.
Qed.

Lemma morphismeSuccesseur_nat_to_bin :
  forall n, nat_to_bin (S n) = S_bin (nat_to_bin n).
Proof.
  fix HR 1.
  intro n.
  case n as [ | pn ].
...
Proof.
  ...
  case n as [ | pn ].
  - reflexivity.
  - case (eq_sym (morphismeSuccesseur_nat_to_bin _)).
    case (eq_sym (morphismeSuccesseur_bin_to_nat _)).
    case (eq_sym (HR pn)).
    reflexivity.
Qed.
...
(* Il exise plusieurs représentations d'un nat en binaire : rien
n'empêche d'ajouter des zéros en tête (c'est-à-dire à droite dans
notre notation binaire. 

binaires b ::= Z | B0 b | B1 b

formes normales fn ::= Z | [B0 + B1]*.B1.Z  

On munit l'ensemble des formes normales d'une structure d'algèbre sur la signature
(Z, B0, B1). Puis on on montre que bin_to_nat et nat_to_bin sont des morphismes d'algèbre.
...
Definition Z_fn : bin := Z. 

Definition B0_fn (b : bin) : bin.
  case b as [ | b0 | b1].
  - exact Z_fn.
  - exact (B0 (B0 b0)).
  - exact (B0 (B1 b1)).
...
Definition B1_fn : bin -> bin := B1.

Fixpoint normalize(b : bin) : bin.
Proof.
  case b as [ | b0 | b1 ].
  - exact Z_fn.
  - exact (B0_fn (normalize b0)).
  - exact (B1_fn (normalize b1)).
Defined.

Compute (normalize (B1 (B0 (B0 Z)))).
Compute (normalize (B0 (B0 (B0 Z)))).

Lemma morphismeZ_bin_to_nat :
  bin_to_nat Z_fn = Z_nat.
Proof.
  reflexivity.
Qed.

Lemma morphismeZ_nat_to_bin :
  nat_to_bin Z_nat = Z_fn.
Proof.
  reflexivity.
Qed.

Lemma morphismeB0_bin_to_nat :
  forall b, bin_to_nat (B0 b) = B0_nat (bin_to_nat b).
Proof.
  intro b.
  case b as [ | b0 | b1 ].
  - reflexivity.
...
Lemma morphismeB0fn_bin_to_nat :
  forall b, bin_to_nat (B0_fn b) = B0_nat (bin_to_nat b).
Proof.
  intro b.
  case b as [ | b0 | b1 ].
  - reflexivity.
...
Lemma morphismeB0_nat_to_bin :
  forall n, nat_to_bin (B0_nat n) = B0_fn (nat_to_bin n).
Proof.
  fix HR 1.
  intro n.
  case n as [ | pn ].
  ...
  - simpl.
    ...
    simpl in HR.
    case (eq_sym (HR pn)).
    case (nat_to_bin pn) as [ | b0 | b1 ].
    + reflexivity.
    + simpl. reflexivity.
    + simpl. reflexivity.
Qed.

Lemma morphismeB1_bin_to_nat :
  forall b, bin_to_nat (B1_fn b) = B1_nat (bin_to_nat b).
Proof.
  intro b.
  case b as [ | b0 | b1 ].
  - reflexivity.
...
Lemma morphismeB1_nat_to_bin :
  forall n, nat_to_bin (B1_nat n) = B1_fn (nat_to_bin n).
Proof.
  fix HR 1.
  intro n.
  case n as [ | pn ].
...
Proof.
  ...
  case b as [ | b0 | b1 ].
  - reflexivity.
  - case (eq_sym (morphismeB0_bin_to_nat _)).
    case (eq_sym (morphismeB0_nat_to_bin _)).
    case (eq_sym (HR _)).
    reflexivity.
  - case (eq_sym (morphismeB1_bin_to_nat _)).
    case (eq_sym (morphismeB1_nat_to_bin _)).
    case (eq_sym (HR _)).
    reflexivity.
Qed.
