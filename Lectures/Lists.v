...
(* REPLACE THIS LINE WITH ":= _your_definition_ ." *) := app.
...
Definition add (v : nat) (s : bag) : bag
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *) := cons v s.
...
(* REPLACE THIS LINE WITH ":= _your_definition_ ." *).
  ...
  - exact nil.
  - case (t =? v).
    + exact (remove_all v r).
    + exact (t :: remove_all v r).
Defined.
...
(* REPLACE THIS LINE WITH ":= _your_definition_ ." *).
  ...
  - case (count t1 s2 =? 0).
    + exact false.
    + exact (subset r1 (remove_one t1 s2)).
Defined.
...
Theorem bag_theorem :
  forall s v, count v (add v s) = S (count v s).
Proof.
  fix HR 1.
  intros s v.
  case s as [ | t r ].
...
Proof.
  ...
  case l1 as [ | t1 r1].
  - simpl.
    case (eq_sym (app_nil_r _)).
    reflexivity.
  - simpl.
    case (eq_sym (HR r1 l2)).
    Search(_ ++ _ ++ _).
    apply app_assoc.
Qed.
...
Proof.
  ...
  intros l1 l2 l3 l4.
  Check app_assoc.
  case (eq_sym (app_assoc (l1 ++ l2) _ _)).
  Check app_assoc.
  case (eq_sym (app_assoc l1 l2 _)).
  reflexivity.
Qed.
...
Theorem bag_count_sum :
  forall s1 s2 v, count v (sum s1 s2) = (count v s1) + (count v s2).
Proof.
  fix HR 1.
  intros s1 s2 v.
  case s1 as [ | t1 r1].
...
Proof.
  ...
  case (rev_involutive l1).
  case (rev_involutive l2).
  case H.
  reflexivity.
Qed.
...
(* FILL IN HERE 
- aucun élément
- uniquement des règles récursives !
*)
