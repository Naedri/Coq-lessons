...
(** **** Exercise: 1 star, standard (plus_id_exercise) 
    ...
    Remove "[reflexivity. Qed.]" and fill in the proof. *)

Theorem plus_id_exercise : forall n m o : nat,
  n = m -> m = o -> n + m = m + o.
Proof.
  (* FILL IN HERE *) 
  intros n m o Enm Emo.
  case Emo.
  case Enm.
  reflexivity.
Qed.
...
(* FILL IN HERE
(* Fixpoint fctn_possible(n : nat) : nat.
      case n as [ | pn].
      - exact 0.
      - exact ((fun n => 0) (fctn_possible (S pn))). (* L'appel interdit disparaît dans le calcul. *)  
Defined.

Fixpoint fctn_impossible(n : nat) : nat.
      case n as [ | pn].
      - exact 0.
      - case (evenb pn). 
        + exact 0.
        + exact (fctn_impossible (S pn)).     
Defined. *)
...
Theorem negb_involutive_extensionnalite :
  forall (f : bool -> bool),
...
(* bin peut être muni d'une structure d'algèbre sur la signature (0, Successeur) :
- zero_bin : interprétation de 0
- S_bin : interprétation du Successeur 
*)

Definition zero_bin : bin := Z.

Definition S_bin := incr.

(* nat peut être muni d'une structure d'algèbre sur la signature (Z, B0, B1). *) 

Definition Z_nat : nat := 0.

Definition B0_nat : nat -> nat := fun n => 2 * n.

Definition B1_nat : nat -> nat := fun n => S (2 * n).

Fixpoint bin_to_nat (m:bin) : nat
  (* REPLACE THIS LINE WITH ":= _your_definition_ ." *). 
  case m as [ | b0 | b1].
  - exact Z_nat.
  - exact (B0_nat (bin_to_nat b0)).
  - exact (B1_nat (bin_to_nat b1)).
Defined.
