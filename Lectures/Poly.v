...
(** Which of the following are well-typed elements of [grumble X] for
    some type [X]?  (Add YES or NO to each line.)
      - [d (b a 5)] : non, il manque X
      - [d mumble (b a 5)] : oui
      - [d bool (b a 5)] : oui
      - [e bool true] : oui
      - [e mumble (b c 0)] : oui
      - [e bool (b c 0)] : non, (b c 0) n'a pas le type bool.
      - [c] : oui  
 *)
(* Pour vérifier le typage d'une expression : Check. *)
...
Proof.
  ...
  case l1 as [ | t1 r1].
  - simpl.
    case (eq_sym (app_nil_r _ _)).
    reflexivity.
  - simpl.
    case (eq_sym (HR _ r1 l2)).
    Search(_ ++ _ ++ _).
    symmetry. (* apply eq_sym. *)
    apply app_assoc.
Qed.
...
Proof.
  ...
  -  reflexivity.
  - simpl.
    case (eq_sym (rev_app_distr _ _ _)).
    case (eq_sym (HR _ r)).
    reflexivity.
Qed.
...
(* REPLACE THIS LINE WITH ":= _your_definition_ ." *).
  Check filter.  
  exact (filter (fun n => andb (evenb n) (7 <? n)) l).
Defined.
...
Definition partition {X : Type}
                   ...
                   : list X * list X
(* REPLACE THIS LINE WITH ":= _your_definition_ ." *).
  exact (filter test l, filter (fun x => negb (test x)) l).
Defined.
...
Proof.
  ...
  - reflexivity.
  - simpl.
    case (eq_sym (morphism_rev _ _ _ (rev r) [t])).
    simpl.
    case (HR _ _ _ r).
    reflexivity.
Qed.
...
pas fait. Se rappeler que les accolades désignent les arguments implicites.
 *)
...
(*
 fold correspond exactement au principe itéraif de récursion. *)
...
Definition tousNuls(l : list nat) : bool :=
  fold (fun n b => if (n =? O) then b else false) l true.

Compute (tousNuls []).
Compute (tousNuls [0]).
Compute (tousNuls [1]).
Compute (tousNuls [1; 0]).
Compute (tousNuls [0 ; 0]).
Compute (tousNuls [0; 1]).
...
Theorem premierDepassement :
  forall X l n, length l = n -> @nth_error X l n = None.
Proof.
  (* Procédons par récurrence sur l. *)
  fix HR 2.
  (* Soient X un type, l une liste de X et n un entier naturel. *)
  intros X l n.
  (* Supponsons que (length l = n) (soit l'hypothèse H).*)
  intro H.
  (* Examinons les deux cas pour l. *)
  case l as [ | t r ].
  - (* cas l := [] *)
    (* Par H, n vaut 0. *)
    case H.
    (* Concluons par réflexivité. *)
    reflexivity.
  - (* cas l := t :: r *)
    (* Par H, n  vaut (S (length r)). *)
    case H.
    simpl.
    (* Appliquons l'hypothèse de récurrence pour conclure. *)
    apply (HR _ r).
    reflexivity.    
Qed.
...
C'est un codage étrange. Il a été inventé par Church pour montrer qu'un langage de programmation minimal, le lambda-calcul, permet de calculer toutes les fonctions calculables, comme une machine de Turing.

*)

Module Church.
Definition cnat := forall X : Type, (X -> X) -> X -> X.
(* cnat := forall X : Type, (X -> X) -> (X -> X). La flèche s'associe à droite. *)
...
(* Plus simplement :   fun (X : Type) (f : X -> X) => f. *)  

(** Similarly, [two] should apply [f] twice to its argument: *)
(* Plus simplement :   fun (X : Type) (f : X -> X) => f o f. (avec o : composition de fonctions. *)
...
(* Plus simplement : fun (X : Type) (f : X -> X) => id. id est le neutre de la composition. *)
...
(* Plus simplement : fun (X : Type) (f : X -> X) => f o f o f. *)
...
(* Plus simplement : fun (X : Type) (f : X -> X) => f o (n X f). *)
...
(* fun (X : Type) (f : X -> X) => (n X f) o (m X f). *)
...
(* solution (notation ([n] : cnat) pour représenter (n : nat)) :
   f -- [n] -> f^n -- [n] -> f^(n^2) -- [n] -> f^(n^3) ... -- [n] -> f^(n^m)
   soit ([m] [n]).
*)
...
(* solution problématique : exp n m := m (mult n) one.
  1 -- * n -> n -- * n -> n^2 ... -- * n -> n^m
...
Definition exp' (n m : cnat) : cnat
 ...
 "Type@{cnat.u0}" (universe inconsistency: Cannot enforce cnat.u0 < cnat.u0 because
cnat.u0 = cnat.u0).
  cnat : Type@{u0 + 1} := forall X : Type@{u0}, (X -> X) -> X -> X. 

  exact (m cnat (mult n) one).
Defined.
*)
...
Module Church_algebrique.
  
Definition cnat : Type := forall (X : Type), (X -> X) -> (X -> X).


(* composition exprimée fonctionnellement *)
Definition seq{X : Type}(f g : X -> X) : X -> X.
  intro x.
  exact (g (f x)).
Defined.

(* notation: [n] pour (puissance n). *)
Fixpoint puissance(n : nat) : cnat.
  intros X f.
  case n as [ | pn].
  - exact (fun x => x).
  - intro x.
    exact ((puissance pn X f) (f x)).
Defined.

Definition zero : cnat := puissance 0.

Definition one : cnat := puissance 1.

Definition two : cnat := puissance 2.

Definition three : cnat := puissance 3.
...
(** Addition of two natural numbers: *)
(* 
f -- [n + m] -> f^(n + m) = f^n ; f^m = ([n] f) ; ([m] f)
*)
Definition plus (n m : cnat) : cnat.
  intros X f.
  exact (seq (n X f)  (m X f)).
Defined.
